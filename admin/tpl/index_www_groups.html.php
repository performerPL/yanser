...
//                  alert(destination.find('li[name='+destination.attr('id')+']').length);
                    ...
                    jQuery.ajax({
                          type: "POST",
                          url: "index_www_groups.php?ajax=1&func=saveGroupOrder&parentId="+parentId,
                          data: data,
                          dataType: "json",
        ...
        jQuery("#groups_tree a[name=add]").live("click",function() {
            ...
            jQuery.post("index_www_groups.php?ajax=1", { func: "saveGroup" , parent_id : parentElem.attr("id")},
            function(data){
                if(data.id > 0) {   
        ...
        jQuery("#groups_tree a[name=delete]").live("click",function() {
            ...
            jQuery.post("index_www_groups.php?ajax=1", { func: "removeGroup" , id : parentElem.attr("id")},
            function(data) {
                if(data.removed) {
        ...
        jQuery("#groups_tree a[name=save]").live("click",function() {
            ...
            jQuery.post("index_www_groups.php?ajax=1", { func: "saveGroupActive,saveGroupName" , id : parentElem.attr("id"), active : active.attr("checked"), name : editName.attr("value")},
            function(data) {
                if(data.saved) {
...
<div class="oper">
  <a href="javascript:void(0)" onclick="document.getElementById('i_cmd').value='new'; forma.submit();" title="Dodaj grupę"><img src="img/icon_user_add_m.gif" border="0" width="20" height="20" alt="" />Dodaj grupę</a>
</div>

<div class="history">
  <img src="img/icon_user.gif" width="64" height="64" border="0" alt="" /> 
  <a href="index.php" title="<?php _t('main_menu'); ?>"><?php _t('main_menu'); ?></a>
  <?php _t('user_www_groups_mgmt'); ?>
</div>
...
foreach ($PATHWAY as $k => $V) {
  ...
  echo '<a href="?PARENT=' . $V['wug_id'] . '">' . $V['wug_name'] . '</a>';
}
$now = (int) $_GET['PARENT'];
$RES = _db_get_one("SELECT wug_name FROM " . DB_PREFIX . "www_user_group WHERE wug_id=" . _db_int($now) . " LIMIT 1");
if (!$first) {
  echo ' / ';
}
echo $RES['wug_name'];
...
<div>Grupy użytkowników:
<ul id="groups_tree" class="simpleTree">
	<li id="0" class="root"><span><a name="add">Dodaj</a></span> <?php echo $generatedGroupList;?>
	</li>
</ul>
</div>
