...
//		        	alert(destination.find('li[name='+destination.attr('id')+']').length);
                    ...
                    jQuery.ajax({
                    	  type: "POST",
                    	  url: "index_www_catalog_groups.php?ajax=1&func=saveGroupOrder&parentId="+parentId,
                    	  data: data,
                    	  dataType: "json",
		...
		jQuery("#groups_tree a[name=add]").live("click",function() {
			...
			jQuery.post("index_www_catalog_groups.php?ajax=1", { func: "saveGroup" , parent_id : parentElem.attr("id")},
			function(data){
				if(data.id > 0) {	
		...
		jQuery("#groups_tree a[name=delete]").live("click",function() {
			...
			jQuery.post("index_www_catalog_groups.php?ajax=1", { func: "removeGroup" , id : parentElem.attr("id")},
			function(data) {
				if(data.removed) {
		...
		jQuery("#groups_tree a[name=save]").live("click",function() {
			...
			jQuery.post("index_www_catalog_groups.php?ajax=1", { func: "saveGroupActive,saveGroupName" , id : parentElem.attr("id"), active : active.attr("checked"), name : editName.attr("value")},
			function(data) {
				if(data.saved) {
	...
	<div class="history">
  	...
  	<?php _t('www_catalog_groups_mgmt'); ?>
	</div>
